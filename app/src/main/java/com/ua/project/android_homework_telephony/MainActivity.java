...
import com.ua.project.android_homework_telephony.data.JsonHelper;
...
//        contactsList = dbHelper.findAll();
        contactsList = jsonHelper.loadContacts();
    ...
    private void fillJson(JsonHelper jsonHelper) {
        if(!getFileStreamPath(jsonHelper.getFileName()).exists()) {
            final List<Contact> CONTACTS_LIST = new ArrayList<>(List.of(
                    Contact.builder().id(1).firstName("FirstName1").phoneNumber("0998877666").build(),
                    Contact.builder().id(2).phoneNumber("0998827116").build(),
                    Contact.builder().id(3).lastName("LastName3").phoneNumber("0668881656").build(),
            ...
            jsonHelper.saveContacts(CONTACTS_LIST);
        }
    }
